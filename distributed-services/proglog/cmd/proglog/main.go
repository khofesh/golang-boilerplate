...
func main() {
	...
	/*
		Cobra calls the RunE function you set on your
		command when the command runs. Put or call the command’s
		primary logic in that function. Cobra enables you to run
		hook functions to run before and after RunE.
	*/
	cmd := &cobra.Command{
		Use:     "proglog",
...
// These flags allow people calling your CLI
// to configure the agent and learn the default configuration
...
// reads the configuration and prepares
// the agent’s configuration. Cobra calls
// setupConfig() before running the command’s RunE function
